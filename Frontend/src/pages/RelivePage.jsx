...
export default function RelivePage() {
  ...
  // Build moments from relive-data (script segments + manifest assets)
  function buildMoments(data) {
    const { script, assets_manifest, tts_output } = data;
    if (!script?.segments?.length) return null;

    const assetMap = {};
    (assets_manifest || []).forEach((a) => { assetMap[a.asset_id] = a; });
    ...
    return script.segments.map((seg, i) => {
      ...
      const asset = firstAssetId ? assetMap[firstAssetId] : null;
      const audio = audioMap[seg.segment_id];

      return {
        id: seg.segment_id,
        time: `Segment ${i + 1}`,
        location: asset?.user_caption || seg.caption_text || 'Memory moment',
        ...
        photos: [asset?.file_url || asset?.url].filter(Boolean),
        audioUrl: audio?.audio_url ? `http://localhost:8000${audio.audio_url}` : null,
        audioDuration: audio?.duration_sec || 0,
        mood: seg.mood,
        day: 1,
        people: [],
        voiceNote: null,
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          // Script not yet generated — fall through to the "generate first" screen
          // or use mock moments if available
  ...
  const handleAssembleAndReload = async () => {
    ...
    try {
      ...
      // Reload relive data after assembly
      const data = await getReliveData(id);
      const moments = buildMoments(data);
