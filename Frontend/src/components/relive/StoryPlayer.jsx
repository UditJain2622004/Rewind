...
import { ChevronLeft, ChevronRight, Pause, Play, List, X, Music, ArrowLeft, Volume2 } from 'lucide-react';
...
export default function StoryPlayer({ moments, memoryTitle, memoryId }) {
  ...
  useEffect(() => {
    setCurrentTime(0);
    setDuration(moment?.audioDuration || moment?.duration || 5.0);
  }, [current, moment]);
  ...
  useEffect(() => {
    ...
    if (isPlaying && audioSrc) {
      audio.play().catch((e) => {
        console.warn("Audio play blocked or interrupted:", e);
      });
    } else {
      audio.pause();
    }
  ...
  return (
    <div className="fixed inset-0 bg-[#060608] z-50 flex items-center justify-center overflow-hidden font-sans">
      ...
      <div className="relative w-full max-w-[430px] h-full sm:h-[92vh] sm:max-h-[860px] aspect-[9/16] sm:rounded-[36px] overflow-hidden bg-black border-0 sm:border border-white/15 shadow-[0_0_60px_rgba(0,0,0,0.8)] flex flex-col justify-between select-none z-20">
        ...
        <div className="relative z-30 pt-3 px-3.5 space-y-2.5">
          ...
          <div className="flex items-center justify-between gap-2 pt-1">
            ...
            <div className="flex items-center gap-1.5">
              {audioSrc && isPlaying && (
                <div className="px-2 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-[10px] text-emerald-300 flex items-center gap-1">
        ...
        {/* LOWER SECTION: Controls & Narration */}
        <div className="relative z-30 px-4 pb-4 pt-10 flex flex-col gap-4">
          
          {/* Spoken Narration Text */}
          {moment.aiNarration && (
            <div className="p-3 rounded-2xl bg-black/50 backdrop-blur-md border border-white/10 text-center">
