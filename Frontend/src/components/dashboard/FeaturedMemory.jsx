...
export default function FeaturedMemory({ memory, items = allMemories }) {
  const slides = items && items.length > 0 ? items : [memory];
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div className="max-w-xl">
                  <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-1 tracking-tight">
                    {currentMemory.fullTitle}
                  </h2>
                  
                  <div className="flex flex-wrap items-center gap-3 text-xs text-white/60 mt-2">
                    <span className="flex items-center gap-1">
                      <Users size={13} />
                      {currentMemory.stats.people} people
                    </span>
                    <span className="w-1 h-1 rounded-full bg-white/20" />
                    <span className="flex items-center gap-1">
                      <Clock size={13} />
                      {currentMemory.stats.days} days
                    </span>
                    <span className="w-1 h-1 rounded-full bg-white/20" />
                    <span className="flex items-center gap-1">
                      <ImageIcon size={13} />
                      {currentMemory.stats.moments} moments
                    </span>
                  </div>
                </div>
